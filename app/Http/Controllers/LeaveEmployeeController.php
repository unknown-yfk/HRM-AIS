...
class LeaveEmployeeController extends Controller
{
    ...
    {
        $leaves = DB::table('leaves_employees')
                    ->join('users', 'users.user_id', '=', 'leaves_employees.user_id')
                    ->select('leaves_employees.*', 'users.position','users.name','users.avatar')
                    ->where('users.id',Auth::user()->id)
                    ->get();

        $anual = LeavesSetting::where('id',1)->get();
        $medical = LeavesSetting::where('id',2)->get();
        $other = LeavesSetting::where('id',3)->get();
    
        return view('form.leavesemployee',compact('leaves','anual','medical','other'));
    }
    // save record
